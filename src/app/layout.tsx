...
export default async function RootLayout({children}: {children: ReactNode}) {
  ...
  // Backwards compatible: older DB rows may have only siteFavicon JSON.
  let faviconMain = (brandingSettings.siteFaviconMain || '').trim();
  let faviconJson: Record<string, string> | null = null;
  if (!faviconMain && (brandingSettings.siteFavicon || '').trim()) {
    try {
      faviconJson = JSON.parse(brandingSettings.siteFavicon) as Record<string, string>;
      faviconMain = (faviconJson.favicon || faviconJson['icon-32'] || '').trim();
    } catch {
      faviconJson = null;
    }
  }

  // If still missing, use /icon (generated by src/app/icon.tsx) which always exists.
  if (!faviconMain) faviconMain = '/icon';

  const favicon192 =
    (faviconJson?.['icon-192'] || '').trim() ||
    (faviconMain.includes('-32x32') ? faviconMain.replace('-32x32', '-192x192') : faviconMain);

  const appleTouchIcon =
    (faviconJson?.['icon-180'] || '').trim() ||
    (faviconMain.includes('-32x32') ? faviconMain.replace('-32x32', '-180x180') : faviconMain);
