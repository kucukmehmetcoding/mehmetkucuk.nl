...
function safePublicRelativePath(urlPath: string): string {
  // Stored values are URL paths like "/uploads/branding/favicon-32x32-...png".
  // Ensure we don't accidentally create an absolute filesystem path.
...
export async function GET(
  ...
  try {
    // For ico, serve the canonical /favicon.ico endpoint.
    if (sizeParam === 'ico') {
    ...
    if (setting?.value) {
      ...
      const iconKey = `icon-${sizeParam}`;
      const iconPath = faviconUrls[iconKey];

      if (iconPath) {
        const fullPath = path.join(process.cwd(), 'public', safePublicRelativePath(iconPath));
    ...
    // If no favicon exists, return 404 (caller can fallback to /icon).
